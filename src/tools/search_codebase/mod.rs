...
pub fn search(project_path: &Path, query: &str, max_results: u32, token_budget: u32, detail: &str) -> Result<String> {
    let max_results = max_results.min(50);
    let token_budget = token_budget.min(100_000);

    let db_path = schema::db_path(project_path);
    if db_path.exists() {
        match indexed::search_indexed(project_path, query, max_results, token_budget, detail) {
            Ok(result) if !result.contains("No results found") => return Ok(result),
            _ => {}
        }
    }

    live::search_live(project_path, query, max_results, token_budget, detail)
}
